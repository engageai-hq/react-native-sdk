...
export function EngageVoiceChatModal({
  ...
  // Press-and-hold refs — track physical press state to beat async race
  ...
  // Stop recording if modal is closed mid-session
  useEffect(() => {
  ...
  const doStopAndProcess = useCallback(async () => {
    ...
    setRiveState(false, false); // idle during processing (not listening)
    try {
      ...
      if (action.message) {
        const audioBase64 = await clientRef.current.synthesizeSpeech(action.message);
        setVoiceStatus('speaking');
        setRiveState(false, true);
  ...
  const handleMicPressIn = useCallback(async () => {
    ...
    try {
      ...
      setRiveState(true, false);
      // Race: finger released before startRecording resolved
  ...
  const characterState: 'idle' | 'listening' | 'thinking' | 'talking' =
        ...
        >
          ...
          >
            ...
            {messages[messages.length - 1]?.isConfirmation && (
              <View style={styles.confirmRow}>
                <TouchableOpacity style={[styles.confirmBtn, { backgroundColor: primaryColor }]} onPress={confirm}>
                  <Text style={styles.confirmBtnText}>Confirm</Text>
                </TouchableOpacity>
                <TouchableOpacity style={[styles.confirmBtn, styles.denyBtn]} onPress={deny}>
          ...
          <View style={styles.inputRow}>
            ...
            <Pressable
              ...
              disabled={status === 'sending' || status === 'executing' || voiceStatus === 'processing'}
              style={[
                styles.micBtn,
...
const styles = StyleSheet.create({
  ...
  characterContainer: { alignItems: 'center', paddingBottom: 8 },
  character: { width: 180, height: 180 },
  characterPlaceholder: { width: 180, height: 180, borderRadius: 90, alignItems: 'center', justifyContent: 'center' },
  characterEmoji: { fontSize: 72 },
  statusChip: { paddingHorizontal: 16, paddingVertical: 6, borderRadius: 20, marginTop: 4 },
  statusText: { fontSize: 13, fontWeight: '500' },
  voiceErrorText: { fontSize: 12, color: '#e53e3e', marginTop: 4, paddingHorizontal: 16, textAlign: 'center' },
  flex: { flex: 1 },
  ...
  bubbleAgent: { backgroundColor: '#f1f1f1', borderBottomLeftRadius: 4 },
  bubbleText: { fontSize: 15, color: '#333', lineHeight: 20 },
  bubbleTextUser: { color: '#fff' },
  confirmRow: { flexDirection: 'row', gap: 12, marginTop: 8, paddingHorizontal: 8 },
  confirmBtn: { flex: 1, paddingVertical: 12, borderRadius: 12, alignItems: 'center' },
  denyBtn: { backgroundColor: '#f1f1f1' },
  confirmBtnText: { color: '#fff', fontWeight: '600', fontSize: 15 },
  denyBtnText: { color: '#666' },
  inputRow: { flexDirection: 'row', alignItems: 'flex-end', paddingHorizontal: 12, paddingVertical: 10, borderTopWidth: 1, borderTopColor: '#eee', gap: 8 },
  textInput: { flex: 1, minHeight: 42, maxHeight: 100, backgroundColor: '#f5f5f5', borderRadius: 21, paddingHorizontal: 16, paddingVertical: 10, fontSize: 15, color: '#333' },
